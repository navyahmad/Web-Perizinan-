...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-700 mt-0.5">
                Pantau seluruh pengajuan izin kantor dan lakukan persetujuan atau penolakan.
            </p>
        </div>
        <div>
            <a href="{{ route('requests.index', ['status' => 'pending_manager']) }}"
                ...
                Menunggu Manager ({{ $stats['pending'] }})
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <div class="bg-white p-5 rounded-xl border border-amber-200 shadow-2xs bg-amber-50/20">
            <span class="text-xs font-semibold text-amber-700 uppercase tracking-wider block">Menunggu Manager</span>
            <div class="mt-2 flex items-baseline justify-between">
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
        ...
        <div class="p-4 border-b border-slate-200 bg-slate-50/50">
            <form method="GET" action="{{ route('admin.dashboard') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                <div>
                ...
                <div class="flex items-center space-x-2">
                    ...
                    @if(request()->hasAny(['search', 'status', 'type', 'department']))
                        <a href="{{ route('admin.dashboard') }}" class="text-xs py-2 px-3 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-lg transition">
                            Reset
                        </a>
                    @endif
                </div>
